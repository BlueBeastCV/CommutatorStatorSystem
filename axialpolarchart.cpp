...
#include "ui_axialpolarchart.h"

axialpolarChart::axialpolarChart(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::axialpolarChart)
{
    ui->setupUi(this);
    myHelper::FormInCenter(this);//窗体居中显示
    this->setWindowFlags(Qt::FramelessWindowHint | Qt::WindowSystemMenuHint);
    ui->polarTitle->set_lab_Title("轴向极坐标图");
...
axialpolarChart::~axialpolarChart()
{
    delete ui;
    isCreated = false;
}

void axialpolarChart::initPolarChart(QWidget *widget, QPolarChart *polarChart, QChartView *view, QSplineSeries *series, QScatterSeries *scatterSeries)
{
...
void axialpolarChart::setRectAera(QRectF rc, int width)
{
...
void axialpolarChart::drawAnnotations(const QRectF &rc)
{
    int x = rc.x();
    int y = rc.y();
    int width = rc.width();
